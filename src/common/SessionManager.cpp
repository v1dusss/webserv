...
void SessionManager::serialize(const std::string &filename) {
    ...
    if (poll(&pfd, 1, 1000) <= 0) { close(fd); return; }
...
void SessionManager::deserialize(const std::string &filename) {
    ...
    if (r != (ssize_t)sizeof(sessionCount)) { close(fd); return; }
    for (size_t i = 0; i < sessionCount; ++i) {
        size_t sidLen = 0;
        if (read(fd, &sidLen, sizeof(sidLen)) != (ssize_t)sizeof(sidLen)) { close(fd); sessions.clear(); return; }
        if (sidLen > 4096) { close(fd); sessions.clear(); return; } // sanity check
        std::string sid(sidLen, '\0');
        if (read(fd, &sid[0], sidLen) != (ssize_t)sidLen) { close(fd); sessions.clear(); return; }
        size_t fileCount = 0;
        if (read(fd, &fileCount, sizeof(fileCount)) != (ssize_t)sizeof(fileCount)) { close(fd); sessions.clear(); return; }
        if (fileCount > 10000) { close(fd); sessions.clear(); return; } // sanity check
        std::vector<std::string> files;
        for (size_t j = 0; j < fileCount; ++j) {
            size_t fnLen = 0;
            if (read(fd, &fnLen, sizeof(fnLen)) != (ssize_t)sizeof(fnLen)) { close(fd); sessions.clear(); return; }
            if (fnLen > 4096) { close(fd); sessions.clear(); return; } // sanity check
            std::string fn(fnLen, '\0');
            if (read(fd, &fn[0], fnLen) != (ssize_t)fnLen) { close(fd); sessions.clear(); return; }
            files.push_back(fn);
        }
        sessions[sid] = files;
    }
